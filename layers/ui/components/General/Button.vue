...
<template>
    <button v-if="isOfferAction" type="button" class="eagle-cta font-font-01 text-sm text-center w-full min-w-0 px-6 py-3.5 no-underline" @click="openOffer(null, data.target || '_blank')">
        {{ data.title }}
    </button>
    <NuxtLink v-else-if="resolvedLink" :to="resolvedLink" class="eagle-cta font-font-01 text-sm text-center w-full min-w-0 px-6 py-3.5 no-underline" :target="data.target || '_self'" :rel="data.rel || ''">
        {{ data.title }}
    </NuxtLink>
