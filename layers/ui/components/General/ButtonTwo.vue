...
<template>
    <button v-if="isOfferAction" type="button" class="eagle-cta-muted font-font-01 text-sm text-center w-fit min-w-32 px-5 py-3.5 no-underline" @click="openOffer(null, data.target || '_blank')">
        {{ data.title }}
    </button>
    <NuxtLink v-else-if="resolvedLink" :to="resolvedLink" class="eagle-cta-muted font-font-01 text-sm text-center w-fit min-w-32 px-5 py-3.5 no-underline" :target="data.target || '_self'" :rel="data.rel || ''">
        {{ data.title }}
    </NuxtLink>
